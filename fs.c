...
iupdate(struct inode *ip)
{
  ...
  dip->nlink = ip->nlink;
  dip->size = ip->size;
  strcpy(dip->password,ip->password);
...
ilock(struct inode *ip)
{
  ...
  if(!(ip->flags & I_VALID)){
    ...
    ip->nlink = dip->nlink;
    ip->size = dip->size;
    strcpy(ip->password,dip->password);
...
bmap(struct inode *ip, uint bn)
{
  ...
  if (bn<NDOUBLEIND) {
    ...
    a = (uint*)bp->data;
    if (a[majorbn] == 0) {
      a[majorbn] = balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);

    uint* a2;
    bp2 = bread(ip->dev,a[majorbn]);
    a2 = (uint*)bp2->data;
    if ((addr = a2[minorbn]) == 0) {
      a2[minorbn] = addr = balloc(ip->dev);
      log_write(bp2);
    }
    brelse(bp2);
    return addr;
